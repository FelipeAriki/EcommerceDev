...
public class DownloadImageForProductQueryHandler : IHandler<DownloadImageForProductQuery, ResultViewModel<Stream>>
{
    ...
    public DownloadImageForProductQueryHandler(IBlobStorageService storageService, IProductRepository productRepository)
    {
    ...
    {
        var productImage = await _productRepository.GetProductImageById(request.IdProductImage);
        if (productImage == null) return ResultViewModel<Stream>.Error("Imagem não encontrada.");
