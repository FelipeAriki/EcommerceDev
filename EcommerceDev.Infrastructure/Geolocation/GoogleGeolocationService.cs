...
public class GoogleGeolocationService : IGeolocationService
{
    ...
    public async Task<int> GetDistance(string origin, string destination)
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Get, $"{_geolocationSettings.ApiBaseUrl}/json?destinations=${destination}&origins={origin}&key={_geolocationSettings.GeolocationApiKey}");
